...
const VALIDATOR_ACTIVITY_STATE_TYPE = 'validator.activity';
...
export class SnapshotStorage {
  ...
  /**
   * Compare current validator activity against the persisted notification state.
   *
   * Rows in notification_state capture the last derived state for a validator
   * for this event family. We queue only on transitions, then update the state
   * in the same transaction so repeated snapshot ticks do not spam users.
   */
  async syncValidatorActivityNotifications(): Promise<{
    ...
    return this.prisma.$transaction(async (tx) => {
      const [currentInactiveRows, previousStates] = await Promise.all([
        tx.validatorsSnapshotStats.findMany({
          where: { isInactive: true },
          select: { validatorIndex: true },
        }),
        tx.notificationState.findMany({
          where: { type: VALIDATOR_ACTIVITY_STATE_TYPE },
          select: { entityKey: true, lastState: true },
        }),
      ]);

      const currentInactive = currentInactiveRows.map((row) => row.validatorIndex);
      const currentInactiveSet = new Set(currentInactive);
      const previousStateMap = new Map(
        previousStates.map((state) => [state.entityKey, state.lastState] as const),
      );

      const newlyInactive = currentInactive.filter(
        (validatorIndex) =>
          previousStateMap.get(this.getValidatorEntityKey(validatorIndex)) !== 'inactive',
      );

      const recovered = previousStates
        .filter(
          (state) =>
            state.lastState === 'inactive' &&
            !currentInactiveSet.has(this.parseValidatorEntityKey(state.entityKey)),
        )
        .map((state) => this.parseValidatorEntityKey(state.entityKey));

      const [inactiveQueued, recoveredQueued] = await Promise.all([
        this.queueValidatorStateNotifications(
          tx,
          newlyInactive,
          VALIDATOR_INACTIVE_NOTIFICATION_TYPE,
        ),
        this.queueValidatorStateNotifications(tx, recovered, VALIDATOR_RECOVERED_NOTIFICATION_TYPE),
      ]);

      if (newlyInactive.length > 0) {
        await Promise.all(
          newlyInactive.map((validatorIndex) =>
            tx.notificationState.upsert({
              where: {
                type_entityKey: {
                  type: VALIDATOR_ACTIVITY_STATE_TYPE,
                  entityKey: this.getValidatorEntityKey(validatorIndex),
                },
              },
              create: {
                type: VALIDATOR_ACTIVITY_STATE_TYPE,
      ...
      return {
        inactiveQueued,
        recoveredQueued,
        stateTransitions: newlyInactive.length + recovered.length,
      };
    });
  }

  private getValidatorEntityKey(validatorIndex: number): string {
    return `validator:${validatorIndex}`;
  }

  private parseValidatorEntityKey(entityKey: string): number {
    return Number(entityKey.replace('validator:', ''));
  }

  private async queueValidatorStateNotifications(
    tx: Prisma.TransactionClient,
    validatorIndexes: number[],
    type: string,
  ): Promise<number> {
    if (validatorIndexes.length === 0) return 0;

    const recipients = await this.getUsersTrackingValidators(tx, validatorIndexes);
    ...
    await tx.notificationQueue.createMany({
      data: recipients.map((recipient) => ({
        ...
        type,
        payload: {
          clusterNames: recipient.clusterNames,
          validatorIndex: recipient.validatorIndex,
  ...
  private async getUsersTrackingValidators(
    tx: Prisma.TransactionClient,
    validatorIndexes: number[],
  ): Promise<Array<{ userId: string; validatorIndex: number; clusterNames: string[] }>> {
    ...
    const grouped = new Map<
      string,
      { userId: string; validatorIndex: number; clusterNames: string[] }
    >();

    for (const cluster of clusters) {
      for (const validator of cluster.validators) {
        const key = `${cluster.ownerId}:${validator.validatorIndex}`;
        const existing = grouped.get(key);

        if (existing) {
          existing.clusterNames.push(cluster.name);
          continue;
        }
