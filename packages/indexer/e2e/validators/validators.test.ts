...
describe('Validators E2E Tests', () => {
  ...
  describe('Validator Creation', () => {
    ...
    it('should save and retrieve validator data correctly', async () => {
      ...
      // The validator row should keep the compact credentials prefix so Electra
      // prefixes like 0x02 remain queryable without storing the full 32 bytes.
      expect(validator2?.withdrawalCredentialsPrefix).toBe('0x01');
    ...
    it('should handle validators with different statuses', async () => {
      ...
      // The compounding credentials prefix must survive ingestion instead of
      // being reduced to only the 20-byte withdrawal address.
      expect(validatorWithdrawn?.withdrawalCredentialsPrefix).toBe('0x02');
    });
  });
  ...
  describe('Validator updates', () => {
    ...
    it('should update validators and mark the epoch when saveValidatorsForEpoch runs', async () => {
      ...
      // The credentials prefix should be updated with the same payload used to
      // derive withdrawalAddress, preserving the prefix for later filtering.
      expect(validator.withdrawalCredentialsPrefix).toBe('0x01');
    ...
    it('should update validators when updateValidators runs', async () => {
      ...
      // The generic update path should persist the prefix too, because it is
      // used when refreshing existing validators outside epoch processing.
      expect(validator.withdrawalCredentialsPrefix).toBe('0x01');
